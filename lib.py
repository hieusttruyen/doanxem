...
def find_image_in_image(screenshot, find_img, method=cv2.TM_CCOEFF_NORMED, threshold=0.8):
    try:
        ...
        loc = np.where(result >= threshold) if method not in [cv2.TM_SQDIFF, cv2.TM_SQDIFF_NORMED] else np.where(result <= threshold)
...
def FindImgInWindow(
    ...
    try:
        ...
        if filtered_locations:
            x = filtered_locations[0][0] + left + h / 2
            y = filtered_locations[0][1] + top + w / 2
            return x, y
        else:
            return None
    except Exception as ex:
